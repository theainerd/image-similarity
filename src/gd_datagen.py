...
def generator_from_df(df, df_overall, headings_dict, batch_size, target_size, features=None,
    ...
    # get Y_L1, Y_L2, attributes binarizers

    # Y_L1 = df_overall.iloc[:,1:2] # second column
    # Y_L1_np = np.array(Y_L1)
    # mlb_L1 = LabelBinarizer()
    # mlb_L1.fit(Y_L1_np.astype(str))

    # Y_L2 = df_overall.iloc[:,2:3] # third column
    # Y_L2_np = np.array(Y_L2)
    # mlb_L2 = LabelBinarizer()
    # mlb_L2.fit(Y_L2_np.astype(str))

    # attributes = df_overall.iloc[:,3:]
    # attributes_np = np.array(attributes)
    # mlb = MultiLabelBinarizer()
    # mlb.fit(attributes_np.astype(str))

    # parametrization
    # parametrization_dict = {'multi_class':[{'category_output':'L1'}, {'sub_category_output':'L2'}], 'multi_label':[{'attribute_output':['bottomwear_fit','jean_type','dress_type','skirt_type','neckline','sleeve_length']}]}
    ...
    while 1:
        ...
        for _ in range(nbatches):
            ...
            try:
                ...
                # print (X.shape)
                # Y = sub.target.values
                # this_Y_L1 = sub.iloc[:,1:2]
                # this_Y_L1_np = np.array(this_Y_L1)
                # this_Y_L1_labels = mlb_L1.transform(this_Y_L1_np)
                # this_Y_L2 = sub.iloc[:,2:3]
                # this_Y_L2_np = np.array(this_Y_L2)
                # this_Y_L2_labels = mlb_L2.transform(this_Y_L2_np)
                # this_attributes = sub.iloc[:,3:]
                # this_attributes_np = np.array(this_attributes)
                # this_attributes_labels = mlb.transform(this_attributes_np)

                # # taking care of the case where only one class needs to be predicted

                # if (len(list(mlb_L1.classes_)) == 1):
                #     this_Y_L1_labels = to_categorical(this_Y_L1_labels)

                # if (len(list(mlb_L2.classes_)) == 1):
                #     this_Y_L2_labels = to_categorical(this_Y_L2_labels)

                # if (len(list(mlb.classes_)) == 1):
                #     this_attributes_labels = to_categorical(this_attributes_labels)

                
                # Y = {'category_output' : this_Y_L1_labels, 'sub_category_output' : this_Y_L2_labels, 'attribute_output' : this_attributes_labels}
